...
"""
BFH Rooms → Google Calendar Sync (Export + MessageCenter + Grid-Fallback + Auto-Login + HTML)

Ziel
- Felder: Von, Bis, Ressource Bezeichnung (Raum), Standortbezeichnung
- Zeitraum: heute → +7 Tage (Europe/Zurich) – im Browser gesetzt, zusätzlich lokal verifiziert/gefiltet
- Export: Toast-/MessageCenter-CSV; Fallback: Grid-Scrape (alle Seiten, PageSize 200)
- Vor dem Push: löscht eigene Events im Fenster (extendedProperties"""
rooms_sync_google.py

- Optional: --split-by standort / gebaeude, und --no-chunk um Tages-Splitting 06–22 abzuschalten
- Zusätzlich: HTML-Tafel (artifacts_sync/schedule.html)
...
USER_DATA_DIR = SCRIPT_DIR / "pw_profile"  # persistentes Profil (kein ständiges Login)
...
def _guess_cols_by_content(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    # Datums-/Zeitspalten abschätzen
    dt_scores = {c: sample[c].apply(try_parse_dt).notna().mean() for c in sample.columns}
    dt_sorted = [c for c, sc in sorted(dt_scores.items(), key=lambda kv: kv[1], reverse=True) if sc >= 0.6]
    ...
    cand_room = [
        c for c, sc in sorted(room_scores.items(), key=lambda kv: kv[1], reverse=True) if sc >= 0.4
    ]
    ...
    site_scores = {
        c: sample[c].astype(str).apply(lambda s: (" - " in s) or bool(addr_pat.search(s))).mean() for c in sample.columns
    }
    cand_site = [c for c, sc in sorted(site_scores.items(), key=lambda kv: kv[1], reverse=True) if sc >= 0.4]
...
def normalize_to_room_times(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(isinstance(c, str) for c in cols):
        df = df.copy()
        df.columns = [f"col{i}" for i in range(len(cols))]
        cols = list(df.columns)
    ...
    if not (col_von and col_bis and col_raum):
        ...
        col_von = col_von or gv
        col_bis = col_bis or gb
        col_raum = col_raum or gr
        col_site = col_site or gs
    ...
    out = out[(out["Von"] <= end) & (out["Bis"] >= start)].copy()
    out["Raumcode"] = out["Raum"].apply(extract_room_code)
    out = out.sort_values(["Von", "Raum"]).reset_index(drop=True)
    return out


def chunk_to_days_6_22(df: pd.DataFrame) -> pd.DataFrame:
    """Schneidet Events ins 7-Tage-Fenster und zerteilt mehrtägige in Tages-Scheiben 06:00–22:00."""
    if df.empty:
        return df
    start, end = compute_window_days_7()
    rows = []
...
def delete_future_own_events(service, calendar_id: str, horizon_days: int = 8) -> None:
    ...
    if to_delete:
        print(f"[GCAL] Alte Events gelöscht: {len(to_delete)}")
...
async def ensure_logged_in(page: Page, timeout_ms: int) -> None:
    """Auto-Login (AzureAD & klassische Form) über ROOMS_USER/ROOMS_PASS. Nutzt persistentes Profil."""
...
async def apply_7day_filter_and_search(page: Page) -> Tuple[bool, bool, bool, str]:
    """Setzt Datum/Zeit (heute → +7, 06:00–22:00), deaktiviert 'Heute…' und drückt 'Finden'."""
    start, end = compute_window_days_7()
    try:
        res = await page.evaluate(
            """(arg) => {
                ...
                // Manuelle Datum/Zeit aktivieren (falls vorhanden)
                ...
                // Buchungszeitpunkt (Heute…): auf "Alle" (0) stellen
                ...
                // Sichtbare Felder setzen (DOM IDs, sonst Fallback)
...
async def extract_kendo_grid(frame: Frame) -> pd.DataFrame:
    ...
    rows = await frame.eval_on_selector_all(
        "div.k-grid-content table tbody tr", "els => els.map(tr => Array.from(tr.children).map(td => td.innerText.trim()))"
    )
    if not rows:
        rows = await frame.eval_on_selector_all(
            "table tbody tr", "els => els.map(tr => Array.from(tr.children).map(td => td.innerText.trim()))"
        )
    if not rows:
        return pd.DataFrame()
    if not headers:
...
async def ensure_window_or_retry(page: Page, timeout_ms: int, attempts: int = 3) -> bool:
    ...
    for i in range(attempts):
        ...
        print(f"Filter gesetzt (Try {i+1}/{attempts}): Von={von_ok} Bis={bis_ok} | Suche ausgelöst={searched} | {tag}")
        await page.wait_for_timeout(1200)
        if await verify_window_matches(page, timeout_ms):
            ...
            break
        try:
            btn_reset = page.locator("button:has-text('Zurücksetzen'), button:has-text('Reset')").first
...
async def _list_messagecenter_reports(page: Page) -> List[str]:
    try:
        msg_url = await page.eval_on_selector(".notification-container.ui-notify", "el => el.getAttribute('data-messageurl')")
    except Exception:
        msg_url = None
    if not msg_url:
    ...
    if isinstance(data, list):
        for m in data:
            html = str(m.get("Message") or m.get("message") or m) if isinstance(m, dict) else str(m)
...
async def _pick_latest_rooms_csv(click_epoch_secs: float, downloads_dir: Path) -> Optional[Path]:
    ...
    print(f"[EXPORT] Datei aus Downloads übernommen: {latest} → {dest}")
    return dest


async def click_export_and_download(page: Page, timeout_ms: int, downloads_dir: Path) -> Optional[Path]:
    try:
        sel_variants = [
            "img[title='Export']",
            "img[alt='Export']",
        ...
        for sel in sel_variants:
            try:
                ...
                continue
        if not btn:
            print("⚠️ Kein Export-Button sichtbar – Fallback via Grid.")
            return None
        ...
        while time.time() - start_ts < min(timeout_ms / 1000, 120):
            ...
            if toast_href:
                try:
                    ...
                    print(f"⚠️ Download über Link fehlgeschlagen: {e}")
                    toast_href = None
            waited = int(time.time() - start_ts)
            if waited - last_log >= 15:
                print(f"[EXPORT] …warte weiterhin auf Export (≈{waited}s)")
                last_log = waited
            await asyncio.sleep(0.5)

        print("[EXPORT] Kein Link – prüfe Downloads-Ordner…")
        waited = 0
        last_log = 0
        while waited < timeout_ms / 1000:
            ...
            if waited - last_log >= 15:
                print(f"[EXPORT] …warte weiterhin auf Export (≈{waited}s)")
                last_log = waited
        print("⚠️ Keine neue CSV im Downloads-Ordner gefunden.")
        return None
    except Exception as e:
        print(f"⚠️ Export nicht möglich: {e} – Fallback via Grid.")
        return None
...
async def run(timeout_ms: int, calendar: str, downloads_override: Optional[str], split_by: str, chunk: bool) -> None:
    downloads_dir = Path(downloads_override) if downloads_override else (Path.home() / "Downloads")
    start, end = compute_window_days_7()
    print(f"🗓️ Zeitraum: {start.strftime('%d.%m.%Y %H:%M')} → {end.strftime('%d.%m.%Y %H:%M')}")
    ...
    async with async_playwright() as p:
        ...
        ok = await ensure_window_or_retry(page, timeout_ms)
        ...
        if dest and dest.suffix.lower() == ".csv":
            try:
                ...
                print(f"⚠️ Konnte CSV nicht parsen: {e}")

        # Fallback: Grid (alle Seiten)
        if raw.empty:
            frame = await wait_for_results_frame(page, timeout_ms)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--timeout", type=int, default=180000, help="Timeout für Export/Grid in Millisekunden")
